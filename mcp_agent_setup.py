...
def setup_agent(logger, working_servers, samples_dir, model_name: str, client: OpenAI, extra_headers: Dict[str, str]):
    ...
    agent_instructions = (
        "You are an agent that can interact with a local filesystem, fetch web content, perform web searches using Brave Search, execute code using the MCP Code Executor, interact with an Obsidian vault using the 'Obsidian MCP Server', and send messages via the Telegram MCP Server.\n"
